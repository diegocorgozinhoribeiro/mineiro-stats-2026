...
def processar_tabela_base(jogos, simulacoes_usuario=None, mapa_logos=None):
    ...
    for jogo in jogos:
        ...
        if gc is not None and gf is not None:
            ...
            if gc > gf:
                if tc_raw in tabela: tabela[tc_raw]['pontos'] += 3; tabela[tc_raw]['v'] += 1
                if tf_raw in tabela: tabela[tf_raw]['d'] += 1
            elif gf > gc:
                if tf_raw in tabela: tabela[tf_raw]['pontos'] += 3; tabela[tf_raw]['v'] += 1
                if tc_raw in tabela: tabela[tc_raw]['d'] += 1
            else:
                if tc_raw in tabela: tabela[tc_raw]['pontos'] += 1; tabela[tc_raw]['e'] += 1
                if tf_raw in tabela: tabela[tf_raw]['pontos'] += 1; tabela[tf_raw]['e'] += 1
        else:
            jogos_abertos.append({'time_casa': tc_raw, 'time_fora': tf_raw})
...
def calcular_probabilidade_exata(tabela_base, jogos_abertos):
    ...
    for cenario in todos_cenarios:
        ...
        for i, res in enumerate(cenario):
            pts_c, pts_f, saldo_c, _ = res
            tc, tf = jogos_para_processar[i]['time_casa'], jogos_para_processar[i]['time_fora']
            ...
            if tf in cenario_state:
                ...
                cenario_state[tf]['sg'] -= saldo_c
                if pts_f == 3: cenario_state[tf]['v'] += 1

        grupos_temp = {'A': [], 'B': [], 'C': []}
        for t_dados in cenario_state.values(): grupos_temp[t_dados['grupo']].append(t_dados)

        ranking_final = []
        for g in keys_groups: grupos_temp[g].sort(key=key_func, reverse=True)
    ...
    for time, cnt in stats_counts.items():
        if div > 0 and (cnt['semi'] > 0 or cnt['inconf'] > 0 or cnt['rebaix'] > 0):
            resultado_final.append({
                'time': time, 'logo': tabela_base[time].get('logo', ''),
                'semi': round((cnt['semi']/div)*100, 1),
                'inconf': round((cnt['inconf']/div)*100, 1),
                'rebaix': round((cnt['rebaix']/div)*100, 1)
            })
...
def api_atualizar():
    ...
    calc = data.get('calcular_probabilidade', False)
    raw_games, logos = obter_dados_memoria()
    tabela_calc, _, jogos_abertos = processar_tabela_base(raw_games, sims, logos)
    classificacao_geral, grupos = ordenar_ranking_final(tabela_calc)
    probs = calcular_probabilidade_exata(tabela_calc, jogos_abertos) if calc else []
    return jsonify({'geral': classificacao_geral, 'grupos': grupos, 'probabilidades': probs})
...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        try: obter_dados_memoria()
        except: print("Offline cache load failed")
